        ...
        <div class="w-full px-6 lg:px-8 xl:px-10 py-4 flex items-center justify-between">
            <div>
                <div class="flex items-center gap-1.5 text-xs text-slate-400 mb-0.5">
                    ...
                    <span class="text-slate-600 font-medium truncate max-w-xs">{{ $product->name }}</span>
                </div>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-12 gap-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <textarea name="description" id="description" rows="4"
                                              ...
                                              class="w-full px-4 py-2.5 border border-slate-300 bg-slate-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none transition-colors">{{ old('description', $product->description) }}</textarea>
                                </div>
                                <div>
                                    ...
                                    <textarea name="preparation_steps" id="preparation_steps" rows="4"
                                              placeholder="Step 1: …"
                                              class="w-full px-4 py-2.5 border border-slate-300 bg-slate-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none transition-colors">{{ old('preparation_steps', $product->preparation_steps) }}</textarea>
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
                        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/60 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-400">Manage ingredients for production cost calculation</p>
                                </div>
                            </div>
                        ...
                        <div class="p-6">
                            ...
                            <div id="ingredients-container" class="space-y-2">
                                @forelse ($product->ingredients as $i => $ing)
                                <div class="ingredient-item grid grid-cols-12 gap-2 sm:gap-3 items-center p-3 rounded-lg bg-slate-50 border border-slate-200">
                                    <div class="col-span-12 sm:col-span-5">
                                        ...
                                        <select name="ingredients[{{ $i }}][ingredient_id]" class="ingredient-select w-full px-3 py-2.5 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 bg-white">
                                            <option value="">— Select Ingredient —</option>
                                            @foreach($ingredients as $ingOpt)
                                            <option value="{{ $ingOpt->id }}"
                                                    data-price="{{ $ingOpt->price }}"
                                                    data-unit="{{ $ingOpt->unit }}"
                                                    {{ $ingOpt->id == $ing->id ? 'selected' : '' }}>
                                                {{ $ingOpt->name }} (₦{{ number_format($ingOpt->price,2) }}/{{ $ingOpt->unit }})
                                            </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-span-5 sm:col-span-3">
                                        ...
                                        <input type="number" name="ingredients[{{ $i }}][quantity]"
                                               value="{{ $ing->pivot->quantity }}" min="0.01" step="0.01"
                                    ...
                                    <div class="col-span-6 sm:col-span-3">
                                        ...
                                        <select name="ingredients[{{ $i }}][unit]" class="unit-select w-full px-3 py-2.5 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 bg-white">
                                            @foreach($uoms as $uom)
                                            <option value="{{ $uom->code }}" {{ $uom->code == $ing->pivot->unit ? 'selected' : '' }}>{{ $uom->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="ingredient-item grid grid-cols-12 gap-2 sm:gap-3 items-center p-3 rounded-lg bg-slate-50 border border-slate-200">
                                    <div class="col-span-12 sm:col-span-5">
                                        <select name="ingredients[0][ingredient_id]" class="ingredient-select w-full px-3 py-2.5 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 bg-white">
                                            <option value="">— Select Ingredient —</option>
                                            @foreach($ingredients as $ingOpt)
                                            <option value="{{ $ingOpt->id }}" data-price="{{ $ingOpt->price }}" data-unit="{{ $ingOpt->unit }}">
                                                {{ $ingOpt->name }} (₦{{ number_format($ingOpt->price,2) }}/{{ $ingOpt->unit }})
                                            </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-span-5 sm:col-span-3">
                                        <input type="number" name="ingredients[0][quantity]" value="1" min="0.01" step="0.01" class="quantity-input w-full px-3 py-2.5 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 bg-slate-50">
                                    </div>
                                    <div class="col-span-6 sm:col-span-3">
                                        <select name="ingredients[0][unit]" class="unit-select w-full px-3 py-2.5 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 bg-white">
                                            @foreach($uoms as $uom)<option value="{{ $uom->code }}">{{ $uom->name }}</option>@endforeach
                                        </select>
                                    </div>
                                    <div class="col-span-1 flex justify-end">
                                        <button type="button" class="remove-ingredient p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
                        ...
                        <div class="p-6">
                            <div id="state-prices-container" class="space-y-3">
                                @foreach ($product->statePrices as $si => $sp)
                                <div class="state-price-item rounded-xl border border-blue-200 bg-blue-50/60 p-4">
                                    <div class="grid grid-cols-1 sm:grid-cols-11 gap-3">
                                        ...
                                        <div class="sm:col-span-3">
                                            ...
                                            <div class="relative">
                                                <span class="absolute inset-y-0 left-3 flex items-center text-slate-400 text-sm pointer-events-none select-none"></span>
                                                <input type="number" name="state_prices[{{ $si }}][discount_price]" value="{{ $sp->discount_price }}" min="0" step="0.01" placeholder="0.00"
                                                       class="w-full pl-8 pr-3 py-2.5 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                                            </div>
                                        </div>
                            ...
                            <div id="state-prices-empty" class="{{ $product->statePrices->count() > 0 ? 'hidden' : '' }} flex flex-col items-center justify-center py-10 text-center">
                ...
                <div class="xl:col-span-4 space-y-5">
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
                        ...
                        <div class="p-5">
                            <div id="img-preview-wrap" class="{{ $product->image_url ? '' : 'hidden' }} mb-4 relative">
                                <img id="img-preview" src="{{ $product->image_url ? asset('storage/'.$product->image_url) : '#' }}" alt="Preview"
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 p-5 space-y-3">
                        ...
                        <button type="button" id="btn-delete-product"
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">
                            You're about to permanently delete <strong class="text-slate-800">{{ $product->name }}</strong>.
...
$(function () {
    ...
    // ingIdx starts after existing rows so indexes don't collide
    let ingIdx = {{ $product->ingredients->count() ?: 1 }};
    let stateIdx = {{ $product->statePrices->count() }};
    ...
    // ── Delete modal ───────────────────────────────────────────────────────
    $('#btn-delete-product').on('click', () => { $('#delete-modal').removeClass('hidden'); $('body').css('overflow','hidden'); });
