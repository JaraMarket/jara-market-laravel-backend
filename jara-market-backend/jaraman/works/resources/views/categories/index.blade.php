    ...
    <div class="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm">
        <div class="w-full px-6 lg:px-8 xl:px-10 py-4 flex items-center justify-between">
            <div>
                ...
                <h1 class="text-xl font-bold text-slate-900">Category Management</h1>
            </div>
    ...
    <div class="w-full px-6 lg:px-8 xl:px-10 py-6 space-y-5">
        ...
        <div class="grid grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-5">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-2">All defined groups</p>
                    </div>
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-5">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">With Products</p>
                        <p class="text-3xl font-bold text-emerald-600 mt-1.5" id="stat-with">—</p>
                        <p class="text-xs text-slate-400 mt-2">Have linked products</p>
                    </div>
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-5">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Empty</p>
                        <p class="text-3xl font-bold text-amber-500 mt-1.5" id="stat-empty">—</p>
                        <p class="text-xs text-slate-400 mt-2">No products assigned</p>
                    </div>
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-5">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-blue-500 mt-1.5" id="stat-products">—</p>
                        <p class="text-xs text-slate-400 mt-2">Across all categories</p>
                    </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-slate-100 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                <div>
                    <h2 class="text-base font-semibold text-slate-800">All Categories</h2>
                    <p class="text-xs text-slate-400 mt-0.5">Search and manage your product categories</p>
                </div>
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <select id="filter-type" class="px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-slate-50 text-slate-700 transition-all">
                        <option value="">All Types</option>
                        @foreach(\App\Models\CategoryType::orderBy('name')->get() as $type)
                ...
                <table id="categories-table" class="w-full text-sm">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            ...
                            <th class="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Type</th>
                            <th class="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Sort Order</th>
                            <th class="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Description</th>
                            <th class="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Products</th>
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">
                            You're about to permanently delete <strong class="text-slate-800" id="modal-category-name"></strong>.
...
$(function () {
    ...
    function loadStats() {
        $.get('{{ route('categories.data') }}', { length: -1, start: 0 }, function (res) {
            const rows = res.data || [];
            let withProducts = 0, empty = 0, totalProducts = 0;
            rows.forEach(r => {
                const c = parseInt(r.products_count ?? 0);
                totalProducts += c;
                if (c > 0) withProducts++; else empty++;
            });
            $('#stat-total').text(res.recordsTotal ?? rows.length);
            $('#stat-with').text(withProducts);
            $('#stat-empty').text(empty);
            $('#stat-products').text(totalProducts);
        });
    }
    loadStats();
    ...
    const table = $('#categories-table').DataTable({
        ...
        ajax: {
            ...
            data: d => { d.search = $('#search').val(); d.type = $('#filter-type').val(); }
        },
        columns: [
            { data: 'DT_RowIndex', orderable: false, searchable: false,
              render: d => `<span class="text-slate-400 text-xs font-mono">${d}</span>` },
            { data: 'name',
              render: d => `<span class="font-semibold text-slate-800">${d}</span>` },
            { data: 'type_name', defaultContent: '—',
              render: d => d && d !== '—' && d !== 'N/A'
                ? `<span class="inline-block px-2 py-0.5 rounded-md text-xs font-medium bg-indigo-50 text-indigo-700 border border-indigo-100">${d}</span>`
                : `<span class="text-slate-300 text-xs">—</span>` },
            { data: 'sort_by',
              render: d => `<span class="text-slate-500 font-mono text-xs">${d ?? '—'}</span>` },
            { data: 'description', defaultContent: '',
              render: d => d
                ? `<span class="text-slate-500 text-xs line-clamp-1 max-w-xs block">${d}</span>`
                : `<span class="text-slate-300 text-xs">—</span>` },
            { data: 'products_count', searchable: false,
              render: d => {
                const n = parseInt(d ?? 0);
                const cls = n > 0 ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-slate-100 text-slate-400 border-slate-200';
                return `<span class="inline-block px-2 py-0.5 rounded-md text-xs font-semibold border ${cls}">${n}</span>`;
              }
            },
            { data: 'created_at',
              render: d => `<span class="text-slate-400 text-xs">${d ?? '—'}</span>` },
            { data: 'actions', orderable: false, searchable: false, className: 'text-right' }
        ],
        language: {
            emptyTable:  'No categories found',
            zeroRecords: 'No categories match your filters',
    ...
    let pendingId = null;
    function openModal(id, name) { pendingId = id; $('#modal-category-name').text(name || 'this category'); $('#delete-modal').removeClass('hidden'); $('body').css('overflow', 'hidden'); }
    function closeModal() { $('#delete-modal').addClass('hidden'); $('body').css('overflow', ''); pendingId = null; }

    $(document).on('click', '.delete-category', function () { openModal($(this).data('id'), $(this).data('name')); });
    ...
    $('#modal-confirm').on('click', function () {
        ...
        $.ajax({
            url: `/categories/${pendingId}`, type: 'DELETE',
            headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
            success: res => { closeModal(); table.ajax.reload(null, false); loadStats(); showToast('Category deleted successfully.', 'success'); },
            error: xhr => { closeModal(); showToast(xhr.responseJSON?.message || 'Delete failed.', 'error'); },
            complete: () => $btn.prop('disabled', false).html('<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg> Delete Category')
        });
    });
