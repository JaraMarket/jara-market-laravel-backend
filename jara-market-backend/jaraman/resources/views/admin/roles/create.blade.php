...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="page-title">{{ isset($admin)?'Edit Admin: '.$admin->name:'Create New Admin' }}</h1>
            <p class="page-subtitle">{{ isset($admin)?'Update role, permissions and profile':'Set up a new admin with role and permissions' }}</p>
        </div>
    </div>

    <form action="{{ isset($admin)?route('admin-management.update',$admin):route('admin-management.store') }}" method="POST" class="space-y-6">
        @csrf @if(isset($admin)) @method('PUT') @endif
        ...
        <div class="card space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2"><label>Password {{ isset($admin)?'(leave blank to keep current)':'' }} @if(!isset($admin))<span class="text-red-500">*</span>@endif</label><input type="password" name="password" {{ !isset($admin)?'required':'' }}></div>
            </div>
        </div>
        ...
        <div class="card space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div id="stateField">
                    <label>Assigned State <span class="text-xs text-slate-400">(required for State Admin)</span></label>
                    <select name="state_id">
                        <option value="">All States (no restriction)</option>
                        @foreach($states as $s)<option value="{{ $s->id }}" {{ old('state_id',$admin->state_id??'')==$s->id?'selected':'' }}>{{ $s->name }}</option>@endforeach
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="card space-y-5">
            <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                <div><h2 class="text-base font-semibold text-slate-900">Permissions</h2><p class="text-xs text-slate-500 mt-0.5">Leave all unchecked to apply role defaults automatically.</p></div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                    ...
                    <label class="flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-200 hover:border-green-400 hover:bg-green-50 cursor-pointer transition-colors">
                        <input type="checkbox" name="permissions[]" value="{{ $perm->id }}" data-slug="{{ $perm->slug }}"
                            class="perm-cb w-4 h-4 rounded text-green-600"
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="px-6 py-2.5 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-xl shadow-sm">{{ isset($admin)?'Save Changes':'Create Admin' }}</button>
...
@push('scripts')
<script>
const roleDescs={super_admin:'Full access to everything in the system.',admin:'Full access to everything in the system.',state_admin:'Access to all activities within their assigned state.',vendor_manager:'Access to vendor database, sales, orders and logistics activities (no finance).',accounts:'Access to all financial dealings — wallet funding, balances, transactions.',audit:'Read-only access to financial data for auditing.',logistics:'Access to order list and delivery management only.'};
...
$('#applyRoleDefaults').on('click',function(){
    ...
    $('.perm-cb').each(function(){$(this).prop('checked',isSuperAdmin||defaults.includes($(this).data('slug')));});
});
