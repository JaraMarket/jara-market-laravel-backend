        ...
        <div class="w-full px-6 lg:px-8 xl:px-10 py-4 flex items-center justify-between">
            <div>
                <div class="flex items-center gap-1.5 text-xs text-slate-400 mb-0.5">
                    ...
                    <span class="text-slate-600 font-medium">Ingredients</span>
                </div>
                <h1 class="text-xl font-bold text-slate-900">Ingredient Registry</h1>
            </div>
    ...
    <div class="w-full px-6 lg:px-8 xl:px-10 py-6 space-y-5">
        ...
        <div class="grid grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-5">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Ingredients</p>
                        <p class="text-3xl font-bold text-slate-900 mt-1.5" id="stat-total">—</p>
                        <p class="text-xs text-slate-400 mt-2">All registered items</p>
                    </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-slate-100 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                <div>
                    <h2 class="text-base font-semibold text-slate-800">All Ingredients</h2>
                    <p class="text-xs text-slate-400 mt-0.5">Search, filter and manage your ingredient inventory</p>
                </div>
            ...
            <div class="w-full overflow-x-auto">
                <table id="ingredients-table" class="w-full text-sm">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            ...
                            <th class="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Ingredient</th>
                            <th class="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Category</th>
                            <th class="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Unit Price</th>
                            <th class="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Unit</th>
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">
                            You're about to permanently delete <strong class="text-slate-800" id="modal-ingredient-name"></strong>.
...
$(function () {
    ...
    function loadStats() {
        $.get('{{ route('ingredients.data') }}', { length: -1, start: 0 }, function (res) {
            const rows = res.data || [];
    ...
    const table = $('#ingredients-table').DataTable({
        ...
        columns: [
            ...
            { data: 'price', orderable: false,
              render: d => `<span class="font-medium text-slate-700">${d}</span>` },
            { data: 'unit',
              render: d => d
                ? `<span class="inline-block px-2 py-0.5 rounded-md text-xs font-medium bg-slate-100 text-slate-600 border border-slate-200">${d}</span>`
                : `<span class="text-slate-300 text-xs">—</span>` },
        ...
        language: {
            ...
            zeroRecords: 'No ingredients match your filters',
            ...
            info: '_START_–_END_ of _TOTAL_ ingredients'
        },
        order: [[2, 'asc']],
    ...
    let pendingId = null;
    function openModal(id, name) { pendingId = id; $('#modal-ingredient-name').text(name || 'this ingredient'); $('#delete-modal').removeClass('hidden'); $('body').css('overflow', 'hidden'); }
    function closeModal() { $('#delete-modal').addClass('hidden'); $('body').css('overflow', ''); pendingId = null; }

    $(document).on('click', '.delete-ingredient', function () { openModal($(this).data('ingredient-id'), $(this).data('ingredient-name')); });
    ...
    $('#modal-confirm').on('click', function () {
        ...
        $.ajax({
            url: `/ingredients/${pendingId}`, type: 'DELETE',
            headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
            success: res => { closeModal(); if (res.success) { table.ajax.reload(null, false); loadStats(); showToast('Ingredient deleted successfully.', 'success'); } else showToast(res.message || 'Could not delete.', 'error'); },
            error: xhr => { closeModal(); showToast(xhr.responseJSON?.message || 'Delete failed.', 'error'); },
            complete: () => $btn.prop('disabled', false).html('<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg> Delete Ingredient')
        });
    });
