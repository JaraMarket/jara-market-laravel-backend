...
<div class="min-h-screen bg-slate-50">
        ...
        <div class="w-full px-6 lg:px-8 xl:px-10 py-4 flex items-center justify-between">
            <div>
                <div class="flex items-center gap-1.5 text-xs text-slate-400 mb-0.5">
                    <a href="{{ route('vendors.index') }}" class="hover:text-emerald-600 transition-colors">Vendors</a>
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                    <span class="text-slate-600 font-medium truncate max-w-xs">{{ $user->business_name ?? $user->firstname.' '.$user->lastname }}</span>
                </div>
                <h1 class="text-xl font-bold text-slate-900">Edit Vendor</h1>
            </div>
            <a href="{{ route('vendors.index') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 text-sm font-medium rounded-lg shadow-sm">
    ...
    <div class="w-full px-6 lg:px-8 xl:px-10 py-6">
        ...
        <form action="{{ route('vendors.update', $user) }}" method="POST">
            @csrf @method('PUT')
                ...
                <div class="xl:col-span-8 space-y-5">
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
                        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/60 flex items-center gap-3">
                            ...
                            <div><h2 class="text-sm font-semibold text-slate-800">Personal Information</h2><p class="text-xs text-slate-400">Vendor contact details</p></div>
                        </div>
                        <div class="p-6 space-y-5">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    <label class="block text-sm font-medium text-slate-700 mb-1.5">First Name</label>
                                    <input type="text" name="firstname" value="{{ old('firstname',$user->firstname) }}" class="w-full px-4 py-2.5 border border-slate-300 bg-slate-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 @error('firstname') border-red-400 @enderror">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="lastname" value="{{ old('lastname',$user->lastname) }}" class="w-full px-4 py-2.5 border border-slate-300 bg-slate-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 @error('lastname') border-red-400 @enderror">
                                    @error('lastname')<p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>@enderror
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Phone Number</label>
                                    <input type="text" name="phone_number" value="{{ old('phone_number',$user->phone_number) }}" class="w-full px-4 py-2.5 border border-slate-300 bg-slate-50 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500 @error('phone_number') border-red-400 @enderror">
                                    @error('phone_number')<p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>@enderror
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Email Address</label>
                                    <input type="email" name="email" value="{{ old('email',$user->email) }}" class="w-full px-4 py-2.5 border border-slate-300 bg-slate-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 @error('email') border-red-400 @enderror">
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
                        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/60 flex items-center gap-3">
                            ...
                            <div><h2 class="text-sm font-semibold text-slate-800">Business Information</h2><p class="text-xs text-slate-400">Business registration details</p></div>
                        </div>
                        <div class="p-6 space-y-5">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Business Name</label>
                                    <input type="text" name="business_name" value="{{ old('business_name',$user->business_name) }}" class="w-full px-4 py-2.5 border border-slate-300 bg-slate-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="business_address" value="{{ old('business_address',$user->business_address) }}" class="w-full px-4 py-2.5 border border-slate-300 bg-slate-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
                                    @error('business_address')<p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>@enderror
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <input type="password" name="password" placeholder="Leave blank to keep current" class="w-full px-4 py-2.5 border border-slate-300 bg-slate-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
                                @error('password')<p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1.5">Confirm Password</label>
                                <input type="password" name="password_confirmation" class="w-full px-4 py-2.5 border border-slate-300 bg-slate-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
                        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/60 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div><h2 class="text-sm font-semibold text-slate-800">Associated Orders</h2><p class="text-xs text-slate-400">Orders linked to this vendor</p></div>
                            </div>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-slate-50">
                                    ...
                                    @empty
                                    <tr><td colspan="5" class="px-5 py-8 text-center text-slate-400 text-sm">No orders linked to this vendor</td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
                ...
                <div class="xl:col-span-4 space-y-5">
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
                        ...
                        <div class="p-5 space-y-3">
                            <div class="flex justify-between text-xs"><span class="text-slate-500">Vendor ID</span><span class="font-mono text-slate-700">#{{ $user->id }}</span></div>
                            ...
                            <div class="flex justify-between text-xs"><span class="text-slate-500">Orders</span><span class="font-semibold text-slate-700">{{ $orders->count() }}</span></div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 p-5 space-y-3">
                        ...
                        <a href="{{ route('vendors.index') }}" class="w-full inline-flex items-center justify-center px-5 py-2.5 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 text-sm font-medium rounded-lg transition-colors">Cancel</a>
                    </div>
                </div>
            </div>
        </form>
