...
class OrderService
{
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | CREATE ORDER — debit wallet via WalletService
    |--------------------------------------------------------------------------
    */
    public function createOrder($request): Order
    {
        return DB::transaction(function () use ($request) {
            ...
            $this->walletService->debit(
                ...
                ownerId   : $order->id,
                ownerType : get_class($order),
                comment   : "Payment for Order #{$order->reference}",
            );
    ...
    /*
    |--------------------------------------------------------------------------
    | CANCEL ORDER — refund wallet via WalletService
    |--------------------------------------------------------------------------
    */
    public function cancelOrder(Order $order): Order
    {
        return DB::transaction(function () use ($order) {
            ...
            $this->walletService->credit(
                ...
                comment   : "Refund for cancelled Order #{$order->reference}",
            );
            ...
            $this->firebase->sendToUser(
                $user,
                'Order Cancelled',
                "Your order #{$order->reference} has been cancelled and refunded.",
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | VENDOR DECISION — accept or reject
    |--------------------------------------------------------------------------
    */
    public function decide(array $data, int $itemId): OrderItem
    {
        ...
        return DB::transaction(function () use ($user, $data, $itemId) {
            $newStatus = $data['status'] === StatusEnum::ACCEPTED()
                ? StatusEnum::PROCESSING()
                : StatusEnum::PENDING();

            $orderItem = OrderItem::with('order.items', 'order.user')->findOrFail($itemId);

            $vendorId = ($user->role === UserPermissionsEnum::ADMIN() && isset($data['vendor_id']))
                ? $data['vendor_id']
                : $user->id;
            ...
            if ($pendingCount === 0) {
                ...
                $order->user->notify(new OrderStatusNotification(
                    $order,
                    OrderNotificationTypeEnum::CUSTOMER(),
                    StatusEnum::PROCESSING()
                ));
                ...
                $this->firebase->sendToUser(
                    $order->user,
                    'Order Processing',
                    "Your order #{$order->reference} is now being processed.",
                    ['type' => 'order_processing', 'order_id' => (string) $order->id]
                );

                $order->items->pluck('vendor')->unique()->filter()->each(
                    fn ($vendor) => $vendor->notify(new OrderStatusNotification(
                        $order,
                        OrderNotificationTypeEnum::VENDOR(),
                        StatusEnum::PROCESSING()
                    ))
                );
            }
    ...
    /*
    |--------------------------------------------------------------------------
    | MARK AS COMPLETED — credit vendors via WalletService
    |--------------------------------------------------------------------------
    */
    public function markAsCompleted(int $id): Order
    {
        return DB::transaction(function () use ($id) {
            ...
            $order->user->notify(new OrderStatusNotification(
                $order, OrderNotificationTypeEnum::CUSTOMER(), StatusEnum::COMPLETED()
            ));

            $this->firebase->sendToUser(
                $order->user,
                'Order Completed',
                "Your order #{$order->reference} has been completed!",
                ['type' => 'order_completed', 'order_id' => (string) $order->id]
            );

            // Credit vendors
            $vendorCredits = $order->items
            ...
            foreach ($vendorCredits as $vendorId => $amount) {
                ...
                $this->walletService->credit(
                    ...
                    amount    : (float) $amount,
                    reference : $order->reference,
                    comment   : "Earnings from Order #{$order->reference}",
                );
            ...
            foreach ($referralCredits as $referralId => $amount) {
                ...
                $this->walletService->credit(
                    ...
                    amount    : (float) $amount,
                    reference : $order->reference,
                    comment   : "Referral commission from Order #{$order->reference}",
                );
            }
    ...
    */
    ...
    private function notifyVendorsOfNewOrder(Order $order): void
    {
        $categoryIds = $order->items()
            ->with('ingredient:id,category_id')
            ->get()
        ...
        $this->firebase->sendToUsers(
            $vendors,
            'New Order Available',
            'A new order matching your categories is available.',
