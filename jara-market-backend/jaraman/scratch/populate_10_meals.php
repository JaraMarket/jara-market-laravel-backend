...
$meals = [
    [
        ...
        'rating' => 4.8,
        'category_ids' => [2, 3], // Protein, Vitamin
...
echo "--- Populating 10 Professional Meals using JaraMarket MCP ---\n";
...
foreach ($meals as $meal) {
    try {
        $request = new Request($meal);
        $response = $tool->handle($request);
        echo $response->content() . "\n";
        $count++;
    } catch (\Exception $e) {
        echo "❌ Error adding [{$meal['name']}]: " . $e->getMessage() . "\n";
    }
}

echo "\n--- Population Finished. Total Added: {$count} ---\n";
